...
def simple_fraud_tool() -> str:
    ...
    try:
        ...
        relatorio = (
            f"Foram encontrados {len(violacoes)} lotes com violações diretas:\n\n"
        )
        for v in violacoes:
...
def complex_fraud_tool() -> str:
    ...
    try:
        ...
        suspeitas = analisar_transacoes_complexas(
            caminho_transacoes, caminho_politica, caminho_emails
        )

        if not suspeitas:
            return (
                "Nenhuma fraude contextual foi identificada nos e-mails e transações."
            )
